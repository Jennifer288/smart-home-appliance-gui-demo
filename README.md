...
## 12. 设备运行日志 / Telemetry Log

页面新增 Telemetry Log 模块，用于记录洗衣机与冰箱的关键状态变化，模拟智能家电中的设备事件采集和状态监控。日志会记录启动、暂停、重置、阶段变化、模式切换、温度调节、开门提醒、高温提醒和告警恢复等事件，并提供洗衣进度、剩余时间、节能评分、告警状态和事件数量等指标。

这个模块用于说明 GUI、控制逻辑和数据监控之间的关系：用户在界面上进行操作，JavaScript demo 层更新设备状态，同时把关键状态变化记录到事件日志中。面试时可以说明：项目不只是做了界面，也考虑了真实智能家电中运行数据的记录、告警追踪和可观察性。
...
- 设备运行日志模块记录关键状态变化，体现事件采集、告警追踪和状态监控意识
...
- 增加设备运行日志模块，记录启动、阶段切换、模式调整、温度变化和告警事件，模拟智能家电状态监控与事件采集。
